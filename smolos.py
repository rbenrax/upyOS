...
class smolOS:
    def __init__(self):
        ...
        try:
            with open(self.name + ".board", "r") as cf:
            ...
            with open(self.name + ".board", "w") as cf:
                cf.write("Board=" + json.dumps(self.board, separators=None) + "\n")
                cf.write("Leds pins=" + json.dumps(self.system_leds_pins, separators=None) + "\n")
                cf.write("Speed range=" + json.dumps(self.cpu_speed_range, separators=None) + "\n")
                cf.write("Turbo=" + json.dumps(self.turbo, separators=None) + "\n")
                cf.write("Aliases=" + json.dumps(self.user_commands_aliases, separators=None) + "\n")
            print("Board config saved")
    ...
    def mv(self,path_o="", path_d=""):
        if path_o == "" or path_d=="":
            ...
            return
        if path_o in self.protected_files:
            self.print_err("Can not move system file!")
        else:
            uos.rename(path_o, path_d)
